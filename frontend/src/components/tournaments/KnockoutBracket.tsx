...
export const KnockoutBracket = ({ matches, onMatchClick, tournamentType }: KnockoutBracketProps) => {
  ...
  if (tournamentType === 'GROUP_STAGE_KNOCKOUT' && matches.some(m => m.phase === 2)) {
    ...
    // Determine the bracket structure based on number of teams advancing
    const phase2Round3 = matches.filter(m => m.phase === 2 && m.roundNumber === 3);

    // Check if this is an 8-team playoff (has quarter-finals)
    const hasQuarterFinals = phase2Round1.length >= 4;

    if (hasQuarterFinals) {
      // 8-team playoff structure (similar to KNOCKOUT)
      const qf1 = phase2Round1.find(m => m.matchNumber === 1);
      const qf2 = phase2Round1.find(m => m.matchNumber === 2);
      const qf3 = phase2Round1.find(m => m.matchNumber === 3);
      const qf4 = phase2Round1.find(m => m.matchNumber === 4);

      const sf1 = phase2Round2.find(m => m.matchNumber === 5);
      const sf2 = phase2Round2.find(m => m.matchNumber === 6);
      const loserSf1 = phase2Round2.find(m => m.matchNumber === 7);
      const loserSf2 = phase2Round2.find(m => m.matchNumber === 8);

      const final = phase2Round3.find(m => m.matchNumber === 9);
      const thirdPlace = phase2Round3.find(m => m.matchNumber === 10);
      const fifthPlace = phase2Round3.find(m => m.matchNumber === 11);
      const seventhPlace = phase2Round3.find(m => m.matchNumber === 12);
