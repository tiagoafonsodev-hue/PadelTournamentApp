...
// We'll test the standings calculation logic directly by extracting it
...
function buildStandingsFromMatches(matches: MockMatch[]): TeamStanding[] {
...
function createMatch(
  ...
  options: { groupNumber?: number; phase?: number; roundNumber?: number; matchNumber?: number } = {}
): MockMatch {
...
function createTieMatch(
  team1: { p1: string; p2: string },
  team2: { p1: string; p2: string },
  score: number, // Both teams have same score
  options: { groupNumber?: number; phase?: number; roundNumber?: number; matchNumber?: number } = {}
): MockMatch {
  return {
    id: `match-${Math.random().toString(36).substr(2, 9)}`,
...
describe('Phase advancement scenarios', () => {
  ...
  describe('KNOCKOUT tournament advancement', () => {
    it('should identify winners and losers from semi-finals', () => {
      const semifinal1 = createMatch(
        { p1: 'p1', p2: 'p2' },
        { p1: 'p3', p2: 'p4' },
        { t1: 6, t2: 4 },
        { roundNumber: 1, matchNumber: 1 }
      );
      const semifinal2 = createMatch(
        { p1: 'p5', p2: 'p6' },
        { p1: 'p7', p2: 'p8' },
        { t1: 3, t2: 6 },
