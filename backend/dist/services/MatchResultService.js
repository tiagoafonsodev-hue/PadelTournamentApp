...
exports.matchResultService = exports.MatchResultService = void 0;
const client_1 = require("@prisma/client");
const TournamentProgressService_1 = require("./TournamentProgressService");
...
class MatchResultService {
    ...
    async submitMatchResult(matchId, result) {
        ...
        const isUpdate = match.status === client_1.MatchStatus.COMPLETED;
        ...
        await TournamentProgressService_1.tournamentProgress.checkAndAdvancePhase(match.tournamentId);
    }
    /**
     * Basic validation - scores must be non-negative
     */
    validateBasicResult(result) {
     ...
     */
    validateTieAllowed(result, match, tournament) {
        const isTie = result.team1Score === result.team2Score;
        if (!isTie)
            return; // Not a tie, no validation needed
     ...
     */
    async updatePlayerStats(playerId, outcome, gamesFor, gamesAgainst, reverse = false) {
        if (outcome === 'draw') {
     ...
     */
    async updatePlayerStatsForTie(playerId, gamesFor, gamesAgainst, reverse = false) {
        const stats = await prisma.playerStats.findUnique({
            where: { playerId },
        });
        if (!stats) {
...
exports.matchResultService = new MatchResultService();
//# sourceMappingURL=MatchResultService.js.map