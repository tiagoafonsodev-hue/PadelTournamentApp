...
exports.tournamentProgress = exports.TournamentProgressService = void 0;
const client_1 = require("@prisma/client");
const TournamentSchedulerService_1 = require("./TournamentSchedulerService");
const TournamentPointService_1 = require("./TournamentPointService");
const prisma = new client_1.PrismaClient();
class TournamentProgressService {
    /**
     * Check if tournament phase is complete and advance if needed
     */
    async checkAndAdvancePhase(tournamentId) {
        ...
        const currentPhaseMatches = tournament.matches.filter((m) => m.phase === tournament.currentPhase);
        const allCompleted = currentPhaseMatches.every((m) => m.status === client_1.MatchStatus.COMPLETED);
        if (!allCompleted)
            return;
        ...
        if (tournament.type === 'ROUND_ROBIN') {
            ...
            await prisma.tournament.update({
                where: { id: tournamentId },
                data: {
                    status: client_1.TournamentStatus.FINISHED,
                    finishedAt: new Date(),
                },
            });
            ...
            const round1Matches = tournament.matches.filter((m) => m.roundNumber === 1);
            const round2Matches = tournament.matches.filter((m) => m.roundNumber === 2);
            const round3Matches = tournament.matches.filter((m) => m.roundNumber === 3);
            const allRound1Complete = round1Matches.length > 0 && round1Matches.every((m) => m.status === client_1.MatchStatus.COMPLETED);
            const allRound2Complete = round2Matches.length > 0 && round2Matches.every((m) => m.status === client_1.MatchStatus.COMPLETED);
            const allRound3Complete = round3Matches.length > 0 && round3Matches.every((m) => m.status === client_1.MatchStatus.COMPLETED);
            ...
            if (round1Matches.length === 4) {
                if (allRound3Complete) {
                    ...
                    await prisma.tournament.update({
                        where: { id: tournamentId },
                        data: { status: client_1.TournamentStatus.FINISHED, finishedAt: new Date() },
                    });
                    await this.updateTournamentStats(tournamentId);
                }
            ...
            if (tournament.currentPhase === 1) {
                ...
                const anyPhase2Completed = phase2Matches.some((m) => m.status === client_1.MatchStatus.COMPLETED);
                ...
                console.log(`  - Round 1 matches: ${round1Matches.length}, completed: ${round1Matches.filter(m => m.status === client_1.MatchStatus.COMPLETED).length}`);
                ...
                const allRound1Complete = round1Matches.length > 0 &&
                    round1Matches.every((m) => m.status === client_1.MatchStatus.COMPLETED);
                const allRound2Complete = round2Matches.length > 0 &&
                    round2Matches.every((m) => m.status === client_1.MatchStatus.COMPLETED);
                ...
                // SECOND: Check if Round 1 is complete and needs Round 2 generation (multi-group)
                    ...
                    else if (round2Matches.every((m) => m.status === client_1.MatchStatus.SCHEDULED)) {
     ...
     */
    async updateTournamentStats(tournamentId) {
        const tournament = await prisma.tournament.findUnique({
            where: { id: tournamentId },
            include: {
                matches: {
        ...
        if (finalPositions.length > 0) {
            await TournamentPointService_1.tournamentPointService.awardTournamentPoints(tournamentId, finalPositions);
        }
    }
    /**
     ...
     */
    async calculateFinalPositions(tournament) {
        const positions = [];
        // Calculate wins per player across all tournament matches
        const completedMatches = tournament.matches.filter(m => m.status === client_1.MatchStatus.COMPLETED);
        ...
        for (const match of completedMatches) {
            ...
            [match.player1Id, match.player2Id, match.player3Id, match.player4Id].forEach(pid => {
                if (!playerWins.has(pid))
                    playerWins.set(pid, 0);
            });
     ...
     */
    async generateKnockoutRound2(tournamentId, round1Matches) {
        ...
        const round2Matches = TournamentSchedulerService_1.tournamentScheduler.generateKnockoutRound2(tournamentId, match1WinnerTeam, match1LoserTeam, match2WinnerTeam, match2LoserTeam);
     ...
     */
    async generateKnockout8TeamRound2(tournamentId, round1Matches) {
     ...
     */
    async generateKnockout8TeamRound3(tournamentId, round2Matches) {
     ...
     */
    async generateSingleGroupRound2(tournamentId, round1Matches) {
     ...
     */
    async generateMultiGroupRound2(tournamentId, round1Matches) {
        ...
        console.log(`[generateMultiGroupRound2] Match details:`, round2Matches.map(m => `Match ${m.matchNumber} (Round ${m.roundNumber})`).join(', '));
        // Create matches in database
        await prisma.match.createMany({
            data: round2Matches,
        });
     ...
     */
    async advanceToPlayoffPhase(tournament) {
        ...
        if (existingPhase2Matches.length > 0) {
            const anyCompleted = existingPhase2Matches.some(m => m.status === client_1.MatchStatus.COMPLETED);
            if (!anyCompleted) {
        ...
        const playoffMatches = TournamentSchedulerService_1.tournamentScheduler.generatePlayoffMatches(tournament.id, team1st, team2nd, team3rd, team4th);
        // Create matches in database
        await prisma.match.createMany({
        ...
        await prisma.tournament.update({
            ...
            data: {
                currentPhase: 2,
                status: client_1.TournamentStatus.PHASE_1_COMPLETE,
            },
        });
    }
    /**
     ...
     */
    async advanceMultiGroupPlayoff(tournament, groupNumbers) {
     ...
     */
    async calculatePhaseStandings(tournamentId, phase) {
        const matches = await prisma.match.findMany({
            where: {
                tournamentId,
                phase,
     ...
     */
    async calculateGroupStandings(tournamentId, groupNumber) {
        const matches = await prisma.match.findMany({
            where: {
                tournamentId,
                groupNumber,
...
exports.TournamentProgressService = TournamentProgressService;
exports.tournamentProgress = new TournamentProgressService();
//# sourceMappingURL=TournamentProgressService.js.map